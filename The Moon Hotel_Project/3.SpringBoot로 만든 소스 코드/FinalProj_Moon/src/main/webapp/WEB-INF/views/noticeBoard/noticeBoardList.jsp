...
<script type="text/javascript">
	$(function(){
		...
		$('#btn1').click(function(){
				if(sys == 2){
					location.href="/noticeBoard/noticeWrite";
				}else{
					alert("관리자만 작성가능합니다.");
...
<div style="height: 800px;">
	...
	<div id="div1" class="container text-center">
							...
							<c:if test="${num>=1 }">
								...
								<tr style="text-align: center">
									...
									<td style="text-align: left">
										<a href="/noticeBoard/detail?no=${vo.noticeNo}">${vo.n_title}</a>
									</td>
									<td>${vo.userid }</td>
									<td><fmt:formatDate value="${vo.n_regdate }" pattern="yyyy-MM-dd" />
									</td>
								</tr>
							</c:if>
						</c:forEach>
		...
		<c:if test="${pageVo.firstPage>1 }">
			<a
				href="<c:url value='/noticeBoard/noticeBoardList?currentPage=${pageVo.firstPage-1}&condition=${map.condition}&keyword=${map.keyword}'/>">
		...
		<c:forEach var="i" begin="${pageVo.firstPage }"
			...
			<c:if test="${i<=pageVo.totalPage }">
				...
				<c:if test="${i!=pageVo.currentPage }">
					<a
						href="<c:url value='/noticeBoard/noticeBoardList?currentPage=${i}&condition=${map.condition}&keyword=${map.keyword}'/>">
						[${i}]</a>
				</c:if>
			</c:if>
		</c:forEach>
		...
		<c:if test="${pageVo.lastPage<pageVo.totalPage }">
			<a
				href="<c:url value='/noticeBoard/noticeBoardList?currentPage=${pageVo.lastPage+1}&condition=${map.condition}&keyword=${map.keyword}'/>">
				<img src="<c:url value='/images/last.JPG'/>">
			</a>
		</c:if>
	</div>
	<form name= "frm1" method="post" action="/noticeBoard/noticeBoardList" style="width: 100%">
		...
		<div class="form-group"style="display:inline-block">
			...
			<br><br>
		<button id="btn1" type="button" class="btn btn-dark" style="float: right; <c:if test="${GuestOrAdmin ne ADMIN_ACCOUNT }">display:none;</c:if>">글쓰기</button>
		<br> <br> <br>
	</div>
</div>
